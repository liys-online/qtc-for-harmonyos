...
void HarmonyHdcTargetsParserTest::stateMapping_data()
{
    ...
    QTest::newRow("Connected_en") << QStringLiteral("Connected") << int(HdcTargetConnectionState::ReadyToUse);
    QTest::newRow("connected_cn") << QStringLiteral("已连接")     << int(HdcTargetConnectionState::ReadyToUse);
    QTest::newRow("Offline_en")   << QStringLiteral("Offline")   << int(HdcTargetConnectionState::ConnectedNotReady);
    ...
    QTest::newRow("booting")      << QStringLiteral("Booting")   << int(HdcTargetConnectionState::Disconnected);
    QTest::newRow("empty")        << QString()                    << int(HdcTargetConnectionState::Disconnected);
    QTest::newRow("garbage")      << QStringLiteral("Xyz!@#")    << int(HdcTargetConnectionState::Disconnected);
}
